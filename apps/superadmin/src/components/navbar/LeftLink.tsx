...
export default function LeftLink({ icon, color, label, tooglesize, path,setOpen }: MainLinkProps) {
  ...
  return (
    <Link className='decoration-transparent' to={path} onClick={()=>{if(setOpen)setOpen(false);}} >
        ...
        className='md:hover:bg-gray-500 w-full hover:bg-slate-200 flex flex-nowrap gap-x-3 whitespace-nowrap'// sm:text-black md:text-white'
        >
            <ThemeIcon color={"gray"} variant="light">
              {icon}
            </ThemeIcon>
            {(!tooglesize || !mediaQuery) && <Text size="sm" color={mediaQuery?"white":'black'} >{label}</Text>}
        </UnstyledButton>
    </Link>
  )
}