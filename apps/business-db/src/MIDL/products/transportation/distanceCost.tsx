...
export default function DistanceCost() {
  ...
  return (
    <div className='bg-white p-3' >
      <Text component={'h1'} align='left' className='px-3'>Distance cost</Text>
      <TextInput
        label="Distance cost per KM"
        className="max-w-md text-left px-3"
        classNames={{ rightSection: 'w-16' }}
        defaultValue={cost?.distanceCostPerKM}
        left="₹"
        value={value ?? cost?.distanceCostPerKM}
        rightSection={
          value && (
                ...
                onClick={async () => {
                  try {
                    ...
                    await updateDoc(docRef, {
                      distanceCostPerKM: value,
                    });
                    dispatch(setCost({ ...cost, distanceCostPerKM: value }));
                    showNotification({
