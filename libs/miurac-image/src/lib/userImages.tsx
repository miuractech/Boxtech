...
const queryLimit = 12;
export default function UserImages({ app, getUrl, setUrl, editMode, cropId }: Props) {
  ...
  return (
    <div>
      {imagesList.length > 0 ? (
        <Grid sx={{ width: '100%', height: 600, overflow: 'auto' }}>
          {imagesList.map((item: any) => (
            <Grid.Col
              ...
              onClick={() => {
                if (editMode) {
                  if(item.cropId === cropId) setUrl({ url: item.url, fileName: item.name });
                  else setUrl({ url: item.url, fileName: item.name });
                } else {
                  getUrl(item.url);
                }
              }}
